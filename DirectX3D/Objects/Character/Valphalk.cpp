...
void Valphalk::EnergyBullets()
{
	...
	if (sequence == 1) // 각도 정했으면 방향 전환함수
	{
		...
		if (RATIO > 0.1f && !playOncePerPattern3)
		{
			Sounds::Get()->Play("em086_05_vo_media_2", 0.5f);
...
void Valphalk::S_HugeStagger()
{
	...
	if (sequence == 1)// 대경직 루프
	{
		...
		{
			Sounds::Get()->Play("em086_05_vo_media_25", 1.5f);
			playOncePerPattern = true;
		}
...
void Valphalk::Patrol()
{
	...
	if (sequence == 4) // 루프해
	{
		...
		// 플레이어가 근처에 있는지 체크해
		if ((target->GlobalPos() - realPos->Pos()).Length() < 4000 && UI->isMapChange)
		{
...
void Valphalk::PushPlayer()
{
	...
	FOR(colliders.size() - 1)
	{
		if (colliders[i]->IsCollision(playerCollider))
		{
			Vector3 dir = (playerCollider->GlobalPos() - colliders[i]->GlobalPos()).GetNormalized();
...
{
	FOR(hupgiFire.size() - 2) hupgiFire[i + 2]->Stop();
}
...
void Valphalk::S_LegAtk()
{
	...
	if (sequence == 2) // 각도 정했으면 방향 전환함수
	{
		...
		if (RATIO > 0.1f && !playOncePerPattern3)
		{
			Sounds::Get()->Play("em086_05_vo_media_1", 0.5f);
			playOncePerPattern3 = true;
		}
	}
...
void Valphalk::S_StabAtk()
{
	...
	if (sequence == 2) // 각도 정했으면 방향 전환함수
	{
		...
		if (RATIO > 0.2 && !playOncePerPattern)
		{
			if (randSound == 0)
			{
				Sounds::Get()->Play("em086_05_vo_media_1", 0.8f);
				playOncePerPattern = true;
			}
			if (randSound == 1)
			{
				Sounds::Get()->Play("em086_05_vo_media_5", 0.8f);
				playOncePerPattern = true;
			}
		}
		...
		{
			playOncePerPattern2 = true;
			Sounds::Get()->Play("em086_05_fx_media_25", 0.4f);
			randSound = 0;
		}
	}
	if (sequence == 3)
	...
	if (sequence == 11) // 각도 정했으면 방향 전환함수
	{
		...
		if (RATIO > 0.2 && !playOncePerPattern)
		{
			...
			if (randSound == 1)
			{
				Sounds::Get()->Play("em086_05_vo_media_5", 0.5f);
				playOncePerPattern = true;
			}
		}
		if (RATIO > 0.8 && !playOncePerPattern2)
		{
			playOncePerPattern2 = true;
			Sounds::Get()->Play("em086_05_fx_media_25", 0.2f);			
			randSound = 0;
		}
	}
...
void Valphalk::S_BackWingAtk()
{
	...
	{
		GetRadBtwTrgt();
		if (radBtwTarget > -XM_PIDIV2 && radBtwTarget <= 0) // 전방 왼쪽 90도
		{
			whichPattern = 1;
			radDifference = radBtwTarget;
		}
		else if (radBtwTarget > -XM_PI && radBtwTarget <= -XM_PIDIV2) // 왼쪽 뒤 90도
		{
			whichPattern = 2;
			radDifference = radBtwTarget + XM_PIDIV2 - 15 * unitRad;
		}
		else if (radBtwTarget > 0 && radBtwTarget <= XM_PIDIV2) // 전방 오른쪽 45도
		{
			whichPattern = 3;
			radDifference = radBtwTarget;
			Scale().x *= -1;
		}
		else if (radBtwTarget > XM_PIDIV2 && radBtwTarget <= XM_PI) // 오른 뒤쪽 90도
		{
			whichPattern = 4;
			radDifference = radBtwTarget - XM_PIDIV2 + 15 * unitRad;
			Scale().x *= -1;
	...
	if (sequence == 2)
	{
		float dist = (realPos->Pos() - target->GlobalPos()).Length() - 1100;

		if(whichPattern == 2)
			Rot().y += radBtwTarget - 15 * unitRad;
		else
			Rot().y += radBtwTarget + 15 * unitRad;
		UpdateWorld();
		vecToTagt = realPos->Pos() + Back() * dist;
		Rot().y -= radBtwTarget;
		UpdateWorld();
		playOncePerPattern3 = false;
		sequence++;
...
void Valphalk::B_SwingAtk() /////////////////// 백스텝 이후 과정 수정 필요
{
	...
	if (sequence == 1) // 각도 정했으면 방향 전환함수
	{
		...
		{
			Sounds::Get()->Play("em086_05_vo_media_26", 0.8f); 
			playOncePerPattern = true;
		}
	}
...
void Valphalk::B_DownBlast() // 옆으로 돌아 측면 폭격
{
	...
	if (sequence == 1) // 각도 정했으면 방향 전환함수
	{
		...
		{
			Sounds::Get()->Play("em086_05_fx_media_32", 0.8f);
			playOncePerPattern2 = true;
		}
	}
...
void Valphalk::E2118()//앞다리 찍기 공격
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_12", 0.8f);
		playOncePerPattern = true;
	}
...
void Valphalk::E2121()//왼쪽 날개 들었다가 찍은다음 살짝 일어나서 다시 자세잡음
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_4", 0.8f); 
		playOncePerPattern = true;
	}
...
void Valphalk::E2171()
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_7", 0.8f);
		playOncePerPattern = true;
	}
...
void Valphalk::E2141()
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_21", 0.8f);
		playOncePerPattern = true;
	}

	if (RATIO>0.19f && !playOncePerPattern2)
	{
		Sounds::Get()->Play("em086_05_vo_media_16", 0.8f);
		playOncePerPattern2 = true;
	}
...
void Valphalk::E2152()
{
	...
	if (RATIO > 0.05 && !playOncePerPattern)
	{
		...
		Sounds::Get()->Play("em086_05_fx_media_32", 0.6f);
		playOncePerPattern = true;
	}
...
//	}
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_23", 0.8f);
		playOncePerPattern = true;
	}
...
void Valphalk::E2211(float degree)//왼쪽 90도 날개찍기 공격동작
{
	...
	if (RATIO < 0.15f)
	{
		RotateToTarget(0.000f, 0.16f);
		Pos() = Lerp(Pos(), vecToTagt, 25 * DELTA);
	}

	if (RATIO > 0.1 && !playOncePerPattern)
	{
		Sounds::Get()->Play("em086_05_se_media_10", 0.8f); 
		playOncePerPattern = true;
	}
...
void Valphalk::EX2265()
{
	...
	if (RATIO > 0.1 && !playOncePerPattern)
	{
		Sounds::Get()->Play("em086_05_fx_media_15", 0.4f);
		playOncePerPattern = true;
	}
	if (RATIO > 0.96)
	{
...
void Valphalk::E2354(float degree) // 풀버스트 전방
{
	...
	if (RATIO > 0.1f && !playOncePerPattern)
	{
		Sounds::Get()->Play("em086_05_vo_media_24", 0.8f);
		playOncePerPattern = true;
	}
	if (RATIO > 0.96)
	{
...
void Valphalk::E2367() // 풀버스트 발사
{
	...
	if (RATIO > 0.5 && RATIO < 0.51)
	{
		if (playOncePerPattern)
		{
			Sounds::Get()->Play("em086_05_fx_media_35", 0.09f);
	...
	if (RATIO > 0.55 && RATIO < 0.56)
	{
		if (!playOncePerPattern)
		{
			Sounds::Get()->Play("em086_05_fx_media_35_2", 0.09f);
...
void Valphalk::EX2374()
{
	...
	if (RATIO > 0.1 && !playOncePerPattern)
	{
		Sounds::Get()->Play("em086_05_vo_media_18", 0.8f);
		playOncePerPattern = true;
	}
...
void Valphalk::E3006() // 큰 데미지 피격
{
	...
	if (RATIO > 0.20 && !playOncePerPattern)
	{
		Sounds::Get()->Play("em086_05_vo_media_29", 1.5f);
		playOncePerPattern = true;
	}
...
void Valphalk::E3015()
{
	...
	if (RATIO > 0.3 && !playOncePerPattern)
	{
		...
		Sounds::Get()->Play("em086_05_vo_media_25", 0.8f);
		playOncePerPattern = true;
	}
	if (RATIO > 0.96)
	{
...
void Valphalk::E3023() // 사망
{
	...
	if (RATIO > 0.44 && !playOncePerPattern)
	{
		Sounds::Get()->Play("questClear", 0.1f);
		UI->valDeath = true;
		Sounds::Get()->Play("em086_05_vo_media_30", 1.8f);
		playOncePerPattern = true;
	}
...
void Valphalk::E3118()
{
	...
	if (RATIO > 0.50 && !playOncePerPattern)
	{
		Sounds::Get()->Play("questClear", 0.1f);
		UI->valDeath = true;
		Sounds::Get()->Play("em086_05_vo_media_30", 1.5f); 
		playOncePerPattern = true;
	}
...
void Valphalk::E4071()
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_20", 0.8f); // 흡기 시작 voice
		Sounds::Get()->Play("em086_05_fx_media_40", 0.6f); // 흡기 시작 sfx
		playOncePerPattern = true;
	}
	if (RATIO > 0.97f)
	{
...
void Valphalk::E22005() // 포효
{
	...
	{
		Sounds::Get()->Play("em086_05_vo_media_10", 0.8f);
		playOncePerPattern = true;
	}
	if (RATIO > 0.96)
	{
