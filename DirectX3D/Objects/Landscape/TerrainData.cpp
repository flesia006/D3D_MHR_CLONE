...
void TerrainData::AlignVertexData()
{
    ...
    for (UINT z = 0; z < height - 1; z++)
    {
        ...
        {
            //인접지 찾기
            UINT index0 = width * z + x;            // 자기 자신
            UINT index1 = width * z + x + 1;        // 오른쪽
            UINT index2 = width * (z + 1) + x;      // 아래쪽(혹은 위쪽)
            UINT index3 = width * (z + 1) + x + 1;  // 대각선

            //인접지 정보를 그대로 벡터에 집어넣는다 (인덱스x)
            // 아래 추가 순서는 렌더할 때 삼각형이 되는 (인덱스 버퍼와 순서가 같아지는) 구성
